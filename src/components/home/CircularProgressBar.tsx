...
const CenterContent: React.FC<{
  ...
  >
    <img
      ...
      draggable={false}
    />
    <span className='text-[16px] font-semibold text-[#4D83FD] leading-none py-[3px]'>
      {title}
    </span>
    <div className='flex items-baseline py-[1.5px]'>
      <span className='text-[64px] font-semibold tracking-[-5px] text-[#4D83FD] pr-[4px] leading-none'>
        {percentage}
      </span>
      <span className='text-[24px] font-semibold text-[#4D83FD] leading-none'>
        %
      </span>
    </div>
    <span className='mt-1 py-[2px] text-[14px] font-normal text-gray-500'>
      {statusText}
    </span>
  </div>
);
        ...
        return (
          ...
          >
            <img
              ...
              className={`${isActive ? 'w-5 h-5' : 'w-2.5 h-2.5'}`}
              draggable={false}
            />
          </button>
        );
...
const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  const innerCircleRadius = ringRadius - CONFIG.dotRadius - 5; // 10px 간격 추가
  ...
  return (
    ...
    >
      ...
      <Indicator count={dotCount} index={dotIndex} onSelect={onDotSelect} />
    </div>
  );
};
