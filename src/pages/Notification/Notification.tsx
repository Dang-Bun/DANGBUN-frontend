...
const Notification: React.FC = () => {
  ...
  const navState =
    (location.state as {
      tab?: string;
      justCreated?: Record<string, unknown>;
    }) || {};
  ...
  const fetchNotifications = async (tab: 'inbox' | 'transmit') => {
    ...
    try {
      let response;
      if (tab === 'inbox') {
        response = await useNotificationApi.listReceived(placeId, {
          page: 0,
          size: 50,
        });
      } else {
        response = await useNotificationApi.listSent(placeId, {
          page: 0,
          size: 50,
        });
      }
      ...
      const notifications: NotificationItem[] = (
        ...
        .map((item: Record<string, unknown>) => {
          ...
          if (tab === 'inbox') {
            ...
            const notificationReceiverId = item.notificationReceiverId as
          ...
          if (tab === 'transmit') {
            ...
            const notificationReceiverId = item.notificationReceiverId as
              | Record<string, unknown>
              | undefined;
            const receiverId = notificationReceiverId?.receiverId as
              | number
              | undefined;
      ...
      try {
        ...
        if (userData) {
          setCurrentUser({
            ...
            name: String(
              userData.name || userData.memberName || userData.userName || '나'
            ),
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (navState.justCreated && placeId) {
      ...
      const optimistic: NotificationItem = {
        ...
        type: 'transmit',
        title: String(
          jc.title ?? (jc.template ? getTitle(String(jc.template)) : '알림')
        ),
  ...
  const handleReadNotification = async (notificationId: string) => {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('notificationStatusChanged', {
          detail: { placeId },
        })
      );
    } catch (error) {
  ...
  return (
    <div className='flex flex-col items-center pt-[60px] pb-[80px] gap-6 bg-white min-h-screen'>
      <Header
        title='알림함'
        showBackButton={true}
        rightElement={
