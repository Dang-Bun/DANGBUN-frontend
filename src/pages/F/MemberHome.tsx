      ...
      prev.map((d) => ({
        ...
        tasks: d.tasks.map((task) => {
          ...
          return {
            ...task,
            isChecked: next,
            completedAt: next ? formatNowHHmm() : undefined,
            completedBy: next ? currentUser : undefined,
          };
        }),
      }))
    );
        ...
        {hasChecklist ? (
          ...
          <section className="w-full mt-6 flex flex-col items-center text-center flex-shrink-0">
            <p className="text-[13px] text-[#99A2AE]">
              다음 체크리스트가 활성화되지 않았습니다.
            </p>
            <p className="text-[13px] text-[#99A2AE] mt-1">
              다음 체크리스트까지 {nextChecklistEta}
            </p>
          </section>
        )}
      </main>
